          ...
          if (board.getComponent(HoverBoardFlag).lastUpdate < date - 1000 * 5) {
            ...
            board.addComponentOrReplace(
              new ecsUtils.RotateTransformComponent(
							  board.getComponent(Transform).rotation, board.getComponent(HoverBoardFlag).initialTransform.rotation, 0.5
							)
            );
          }
        }
  ...
  addBoard(position: Vector3, rotate: number): Entity {
    ...
    board.addComponent(
      new AvatarModifierArea({
        ...
        //area: { box: transform.scale.clone() },
    ...
    //// Show custom avatar
    //board.addComponent(
    //  new ecsUtils.TriggerComponent(
    //    new ecsUtils.TriggerBoxShape(
    //			new Vector3(2,2,2),
    //			//Vector3.Zero()
    //		),
    //    {
